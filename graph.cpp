...
vector<Vertex> Graph::getIncomingVertices(Vertex source) const {
    auto lookup = in_adjacency_list.find(source);
    if (lookup == in_adjacency_list.end()) return vector<Vertex>();
    else return in_adjacency_list[source];
}
...
bool Graph::insertEdge(Vertex source, Vertex destination)
{
    ...
    if(adjacency_list.find(source)==adjacency_list.end()) // source vertex doesn't exist
    {
        adjacency_list[source] = unordered_map<Vertex, Edge>(); // create a new vertex first
    }
    if (in_adjacency_list.find(destination) == in_adjacency_list.end()) {
        in_adjacency_list[destination] = vector<Vertex>();
    }
    // now source vertex must exist
    adjacency_list[source][destination] = Edge(source, destination);
    in_adjacency_list[destination].push_back(source);
    if(!directed)
    {
...
Edge Graph::removeEdge(Vertex source, Vertex destination)
{
    ...
    vector<Vertex> & adj = in_adjacency_list[destination];
    for (int i = 0; i < (int) adj.size(); i++) {
        if (source.compare(adj[i])) {
...
int Graph::getId(Vertex vertex) const{ 

string str_index = vertex; 
